...
function App() {
  ...
  useEffect(() => {
    const updateCanvasSize = () => {
      ...
      const maxHeight = window.innerHeight - 300; // Space for UI elements
      
      // Use the smaller dimension to maintain square aspect ratio
      const size = Math.min(Math.min(800, maxWidth), Math.min(800, maxHeight));
      
      // Ensure minimum playable size
      const finalSize = Math.max(size, 600);
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="text-center mb-6 relative z-10">
        <h1 className="text-4xl md:text-7xl font-black bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text text-transparent mb-4 tracking-wider">
          BREAKOUT
        </h1>
        <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6 max-w-md mx-auto">
          <p className="text-gray-300 text-sm md:text-lg font-medium">
            A futuristic take on the classic arcade game. Break all the blocks and advance through levels!
          </p>
        </div>
      </div>
