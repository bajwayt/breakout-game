...
function App() {
  ...
  useEffect(() => {
    const updateCanvasSize = () => {
      ...
      const maxHeight = window.innerHeight - (isMobile ? 180 : 300); // Less space for UI on mobile
      ...
      const minSize = isMobile ? 350 : 600;
    ...
    <div className="main-container">
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-2 md:p-4 relative overflow-hidden">
        ...
        <div className="text-center mb-2 md:mb-6 relative z-10">
          <h1 className={`${isMobile ? 'text-2xl' : 'text-4xl md:text-7xl'} font-black bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text text-transparent mb-1 md:mb-4 tracking-wider`}>
            BREAKOUT
          </h1>
          <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-2 md:p-6 max-w-md mx-auto">
            <p className={`text-gray-300 ${isMobile ? 'text-xs' : 'text-sm md:text-lg'} font-medium`}>
              {isMobile ? 'Touch & drag to control!' : 'A futuristic take on the classic arcade game. Break all the blocks and advance through levels!'}
            </p>
          </div>
        </div>
        ...
        <div className={`relative ${screenShake ? 'screen-shake' : ''} mb-2 md:mb-0`}>
          <GameCanvas
